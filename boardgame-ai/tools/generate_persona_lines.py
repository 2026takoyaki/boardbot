...
- 아래 고유명사는 게임에서 정해진 이름이다. 화면 카드·점수판에 적힌 그대로여야
  한다 — 다른 낱말로 바꾸면 같은 뜻이라도 화면과 음성이 다른 이름을 말하게 된다.
  비슷한 말로 순화하거나 대체하지 말고 정확히 이 표기를 쓴다.
    늑대인간 역할명: 도플갱어, 늑대인간, 하수인, 예언자, 강도, 말썽쟁이,
      주정뱅이, 불면증환자, 무두장이, 사냥꾼, 프리메이슨, 마을주민
    요트 족보명: 요트, 풀 하우스, 스몰 스트레이트, 라지 스트레이트
      (영문 Full House, S. Straight, L. Straight 등은 위 규칙대로 그대로 둔다)
  예: "강도"를 "도둑"이라고 하면 안 된다. "말썽쟁이"를 "말썽꾼"이라고 하면
  안 된다. 둘 다 이 게임에 없는 역할이 된다.
- 빈 문자열은 빈 문자열 그대로.
