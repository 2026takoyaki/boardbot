...
_WEREWOLF: dict[str, str] = {
    ...
    # 카운트다운은 "언제부터 지목할 수 있는가"가 아니라 "언제까지 지목할 수
    # 있는가"다. 5초 동안 계속 바꿀 수 있고 끝나는 순간에 확정된다. 예전 문구는
    # "카운트다운이 시작되면 지목해주세요"라 5초가 다 지난 뒤에 한 번 손을 들라는
    # 말로 들렸고, 실제로 다들 마지막에 손을 들어 지목을 놓쳤다.
    "vote_intro":        "5초 안에 투표할 사람을 지목하세요. 도중에 바꿔도 됩니다.",
    ...
    "vote_count_2":      "이",
    "vote_count_1":      "일",
    # 0은 "지금 지목하라"가 아니라 "지목은 여기까지"다. 앞의 안내와 짝이 맞아야
    # 5초 내내 겨누다가 끝에서 확정된다는 것이 들린다.
    "vote_count_0":      "투표 종료!",
}
...
_STRATEGY: dict[str, str] = {
    ...
    #
    # **조언만 남기고 규칙 설명은 버린다 (25자 이내).**
    #
    # 원래는 "예언자는 다른 플레이어 카드 한 장 또는 센터 카드 두 장을 볼 수
    # 있습니다. 의심스러운…"처럼 55~75자였는데, 앞의 절반은 바로 직전에 진행
    # 안내가 이미 한 말이다. 같은 말을 두 번 하느라 야간 단계의 발화 예산을
    # 다 쓰고, 뒤에 나와야 할 "눈을 다시 감아주세요"가 밀려 나오지 못했다.
    # 규칙은 안내가 말하고 화면 카드에도 적혀 있다. 여기 남을 것은 조언뿐이다.
    "ww_doppelganger": "예언자나 강도를 노리세요.",
    "ww_seer": "의심스러운 사람의 카드를 보세요.",
    "ww_robber": "강해 보이는 사람의 카드를 노리세요.",
    "ww_troublemaker": "의심스러운 사람의 카드를 바꿔 혼란을 주세요.",
    "ww_drunk": "새 역할을 모르니 낮에는 신중하게 말하세요.",
    "ww_insomniac": "카드가 바뀌었다면 누가 손을 댄 겁니다.",
}
