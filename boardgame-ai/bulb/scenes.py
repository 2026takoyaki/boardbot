...
# 조정은 tools/tune_dot_counter.py 의 색온도 트랙바로 눈으로 보며 한다.
#
# 상한(6500K)까지 올리지 않고 한 칸 남겨둔다. 역전 Cue가 "중립보다 서늘한
# 번쩍임"이라 중립이 상한에 붙으면 그보다 차가운 색이 없어져 연출이 통째로
# 사라진다 — 색온도가 붙은 명령은 RGB를 쓰지 않으므로 color 값이 달라도
# 전구 출력은 같아진다.
NEUTRAL_KELVIN = 5700
...
#   upset     6500K   역전 — 중립보다 서늘한 번쩍임 (전구 상한)
